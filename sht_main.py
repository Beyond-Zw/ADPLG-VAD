...
if __name__ == "__main__":
    ...
    normal_train_loader = DataLoaderX(
        SHT(root_dir=config.root_dir, mode='Train', modal=config.modal, num_segments=config.num_segments,
                len_feature=config.len_feature, is_normal=True),
        batch_size=config.batch_size,
        shuffle=True, num_workers=config.num_workers,
        worker_init_fn=worker_init_fn, drop_last=True)
    abnormal_train_loader = DataLoaderX(
        SHT(root_dir=config.root_dir, mode='Train', modal=config.modal, num_segments=config.num_segments,
                len_feature=config.len_feature, is_normal=False),
        batch_size=config.batch_size,
        shuffle=True, num_workers=config.num_workers,
        worker_init_fn=worker_init_fn, drop_last=True)
    ...
    for step in tqdm(
        ...
        cost_v, loss_cls_s, lost_triplet, loss_cls_v, cos_simi_loss = train(net_v, net_s, normal_loader_iter,
                                                                            abnormal_loader_iter, optimizer_v,
                                                                            optimizer_s, step, config, pl_dict,
                                                                            scheduler_s)
        if step % 50 == 0:
            print(f"[Step {step}] Total loss: {cost_v.item():.4f}, VC Loss: {loss_cls_v.item():.4f}, Triplet: {lost_triplet.item():.4f}, SC Loss: {loss_cls_s.item():.4f}, COS Loss: {cos_simi_loss.item():.4f}")
        if step % 1000 == 0 and step > config.warm_up:
            test(net_s, test_loader, test_info, step, model_file=None)
            torch.save(net_s.state_dict(), os.path.join(args.model_path, f"sht_model_s_{config.ID}_step_{step}_auc_{test_info['auc'][-1]:4f}.pkl"))
            torch.save(net_v.state_dict(), os.path.join(args.model_path, f"sht_model_v_{config.ID}_step_{step}_auc_{test_info['auc'][-1]:4f}.pkl"))
