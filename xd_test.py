...
def test(net, test_loader, test_info, step, model_file = None):
    with torch.no_grad():
        ...
        frame_gt = np.load("frame_label/xd-gt.npy")
        frame_predict = None
        cls_label = []
...
if __name__ == "__main__":
    ...
    test_loader = DataLoaderX(
        XDVideo(root_dir=config.root_dir, mode='Test', modal=config.modal, num_segments=config.num_segments,
    ...
    test(net_s, test_loader, test_info, step, model_file)
