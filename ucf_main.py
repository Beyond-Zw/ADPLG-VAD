...
if __name__ == "__main__":
    ...
    normal_train_loader = DataLoaderX(
        UCF_crime(root_dir=config.root_dir, mode='Train', modal=config.modal, num_segments=config.num_segments,
    ...
    #  normal_train_loader = DataLoaderX(
    #     UCF_crime_CLIP(root_dir=config.root_dir, mode='Train', modal=config.modal, num_segments=config.num_segments,
    #             len_feature=config.len_feature, is_normal=True),
    #     batch_size=config.batch_size,
    #     shuffle=True, num_workers=config.num_workers,
    #     worker_init_fn=worker_init_fn, drop_last=True)
    ...
    #     worker_init_fn=worker_init_fn, drop_last=True)
    # test_loader = DataLoaderX(
    #     UCF_crime_CLIP(root_dir=config.root_dir, mode='Test', modal=config.modal, num_segments=config.num_segments,
    #             len_feature=config.len_feature),
    #     batch_size=1,
    #     shuffle=False, num_workers=config.num_workers,
    #     worker_init_fn=worker_init_fn)
    ...
    for step in tqdm(
        ...
        if step % 1000 == 0 and step > config.warm_up:
            ...
            torch.save(net_s.state_dict(), os.path.join(args.model_path, f"ucf_model_s_{config.ID}_step_{step}_auc_{test_info['auc'][-1]:4f}.pkl"))
            torch.save(net_v.state_dict(), os.path.join(args.model_path, f"ucf_model_v_{config.ID}_step_{step}_auc_{test_info['auc'][-1]:4f}.pkl"))
